...
describe("ProjectHeader", () => {
  ...
  it("renders a Next link when nextId is provided", () => {
    ...
    expect(
      screen.getByRole("link", { name: "Next Project" })
    ).toHaveAttribute("href", "/projects/proj-3");
  });

  it("renders no Previous link when prevId is absent", () => {
    render(<ProjectHeader title="T" path="projects" nextId="proj-3" />);
    expect(
      screen.queryByRole("link", { name: "Previous Project" })
  ...
  it("renders no Next link when nextId is absent", () => {
    render(<ProjectHeader title="T" path="projects" prevId="proj-1" />);
    expect(
      screen.queryByRole("link", { name: "Next Project" })
    ).not.toBeInTheDocument();
  });
  ...
  it("uses the path prop to construct hrefs", () => {
    ...
    expect(
      screen.getByRole("link", { name: "Previous Project" })
    ).toHaveAttribute("href", "/services/svc-0");
    expect(
      screen.getByRole("link", { name: "Next Project" })
    ).toHaveAttribute("href", "/services/svc-2");
  });
});
